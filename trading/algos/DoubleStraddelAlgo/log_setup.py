...
# Line timestamping
# --------------------------------------------------------------------------- #
# print() may reach the stream in several partial writes (the text, then the
# trailing "\n"), and multiple threads print concurrently. We track whether the
# next character starts a fresh line and, under a shared lock, prefix every
# non-empty line with [HH:MM:SS] so the console, log file and Telegram all get
# consistently timestamped output.
_stamp_lock = threading.Lock()
_at_line_start = True


def _stamp(message):
    """Return `message` with a [HH:MM:SS] prefix on each non-empty line."""
    if not message:
        return message
...
class _Tee:
    ...
    def write(self, message):
        # Prefix each line with a wall-clock timestamp so every log entry is
        # timed (console, file and Telegram all share the same stamped text).
        message = _stamp(message)
