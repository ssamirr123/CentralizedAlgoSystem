...
def exit_leg(symbol, token, qty, reason):
    ...
    pnl = round((entry_price - exit_price) * exit_qty, 2)   # short leg P&L, positive = profit

    # Net realized ledger: a losing exit (pnl<0) ADDS to cum_loss; a
    # profitable exit (pnl>0) SUBTRACTS from it, so a realized gain on one
    # leg genuinely offsets a realized loss on the other in _combined_loss()
    # below, instead of being dropped on the floor.
    config.cum_loss[token] = config.cum_loss.get(token, 0) - pnl
...
def _leg_unrealized_pnl_as_loss(token, qty):
    """Current mark-to-market P&L on this leg if it's open, else 0, expressed
    as a "loss" figure: positive = losing, negative = a live unrealized
    PROFIT (short option: loses when the premium rises above entry, gains
    when it falls below entry). NOT floored at zero -- an open leg that is
    currently profitable returns a negative number here, so it nets against
    (offsets) the other leg's loss in _combined_loss() below, instead of
...
def _combined_loss(ce_token, pe_token, qty):
    """Rules 1-3: NET combined CE+PE premium P&L per lot, expressed as a
    "loss" figure (positive = net loss, negative = net profit) -- realized
    P&L already booked today on either leg (a profitable realized exit
    SUBTRACTS from this total), plus live unrealized MTM on whichever
    leg(s) are currently open (a leg currently in profit also SUBTRACTS).
    A gain on one leg genuinely offsets a loss on the other; this is a true
    net figure, not a sum of only the losing leg(s)."""
    realized = config.cum_loss.get(ce_token, 0.0) + config.cum_loss.get(pe_token, 0.0)
    unrealized = _leg_unrealized_pnl_as_loss(ce_token, qty) + _leg_unrealized_pnl_as_loss(pe_token, qty)
...
def _current_combined_premium(ce_token, pe_token):
    """Live CE+PE premium right now (last_ltp sum) -- NOT the same figure as
    combined_loss below. This is just today's current straddle premium for
    visibility; combined_loss is the net P&L figure (see its own
    docstring)."""
    ce_ltp = config.last_ltp.get(ce_token)
    pe_ltp = config.last_ltp.get(pe_token)
...
def check_combined_risk(ce_symbol, ce_token, pe_symbol, pe_token, qty):
    ...
    NOTE ON "combined": combined_loss is a true NET P&L figure (positive =
    net loss, negative = net profit) across both legs, realized + live
    unrealized. A leg currently in profit SUBTRACTS from the other leg's
    loss instead of contributing nothing -- see _leg_unrealized_pnl_as_loss()
    and _combined_loss() for the exact mechanics.
    """
    ...
    combined_premium = _current_combined_premium(ce_token, pe_token)
    ce_pnl = _leg_unrealized_pnl_as_loss(ce_token, qty)
    pe_pnl = _leg_unrealized_pnl_as_loss(pe_token, qty)
    detail = (f'combined_premium={combined_premium} '
              f'ce_ltp={config.last_ltp.get(ce_token)} ce_pnl_contrib={ce_pnl:.2f} '
              f'pe_ltp={config.last_ltp.get(pe_token)} pe_pnl_contrib={pe_pnl:.2f}')
    ...
    if level_index >= len(config.RISK_LOSS_LEVELS) - 1:
        ...
        print(f'[RISK] Rule 3: combined loss {combined_loss:.2f} >= {threshold:.2f} '
              f'- exiting BOTH legs, no more trading today ({detail})')
