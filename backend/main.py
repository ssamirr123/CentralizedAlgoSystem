...
Deployed separately from the Streamlit dashboard (see api/index.py,
vercel.json). EC2 workers POST to: https://<your-vercel-app>/update_strategy

NOTE: in-memory storage does not reliably persist across Vercel serverless
invocations/cold starts — heartbeats can be lost. Swap in backend/database.py
+ backend/models.py (SQLAlchemy, DATABASE_URL) when persistence is needed.
"""
